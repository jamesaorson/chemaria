	...
	function platypus.update(dt)
		...
		if platypus.parent_id then
			...
			if not ok then
				...
				platypus.parent_id = nil
				state.current.ground_contact = false
				local newPosition = currentPosition + state.current.world_position + BOUNDS_BOTTOM
				if state.current.world_position == state.current.position then
					newPosition = newPosition - currentPosition
				else
					newPosition = newPosition - (state.current.position / 3)
				end
